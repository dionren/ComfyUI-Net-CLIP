# This file is a clone from https://github.com/biegert/ComfyUI-CLIPSeg
...
class ComfyUINetCLIPSeg:
    ...
    def segment_image(self, image: torch.Tensor, text: str, blur: float, threshold: float, dilation_factor: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        model_full_path = os.path.join(folder_paths.models_dir, "clip/clipseg-rd64-refined")
        print(model_full_path)

        processor = CLIPSegProcessor.from_pretrained(model_full_path)
        model = CLIPSegForImageSegmentation.from_pretrained(model_full_path)
